...
public class DepositTransferServiceImpl implements DepositTransferService {
    ...
    @Override
    public Flux<DepositTransferDto> getPaginatedDepositTransfer(String userSupabaseId, Integer page, Integer size, String sortBy) {
        ...
        return userProfileService.getUserProfileBySupabaseId(UUID.fromString(userSupabaseId))
                ...
                .doOnComplete(() -> log.info("Completed fetching deposit transfers for user: {}", userSupabaseId))
                .doOnError(e -> log.error("Failed to fetch deposit transfers: {}", e.getMessage(), e));
    }

    @Override
    public Mono<DepositTransferDto> getASingleDepositTransfer(Long id, String userSupabaseId) {
        return userProfileService.getUserProfileBySupabaseId(UUID.fromString(userSupabaseId))
                .switchIfEmpty(Mono.error(new ResourceNotFoundException("User profile not found")))
                .flatMap(up ->
    ...
    @Override
    public Mono<Void> deleteDepositTransfer(Long id, String userSupabaseId) {
        UUID supabaseUuid;

        try {
            supabaseUuid = UUID.fromString(userSupabaseId);
        } catch (IllegalArgumentException e) {
            return Mono.error(new IllegalArgumentException("Invalid Supabase ID format"));
        }

        return userProfileService.getUserProfileBySupabaseId(supabaseUuid)
                .switchIfEmpty(Mono.error(new ResourceNotFoundException("User profile not found")))
                .flatMap(userProfile ->
    ...
    public Mono<DepositTransferDto> createDepositTransfer(
        ...
        Mono<DepositTransferDto> transferMono = userProfileService.getUserProfileBySupabaseId(supabaseUuid)
                ...
                .flatMap(senderProfile ->
                        ...
                        walletService.getWalletByUserProfileId(senderProfile.getId())
                                ...
                                .flatMap(senderWallet ->
                                        ...
                                        userProfileService.getUserEmail(depositTransferDto.getEmail())
        ...
        return this.transactionalOperator.transactional(transferMono)
                .doOnSubscribe(s -> log.info("Starting deposit transfer for Supabase user: {}", userSupabaseId))
