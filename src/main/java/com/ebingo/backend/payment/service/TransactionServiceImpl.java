...
public class TransactionServiceImpl implements TransactionService {
    ...
    public Flux<TransactionDto> getPaginatedTransaction(String userSupabaseId, Integer page, Integer size, String sortBy) {
        ...
        return userProfileService.getUserProfileBySupabaseId(UUID.fromString(userSupabaseId))
                ...
                .doOnComplete(() -> log.info("Completed fetching transactions for user: {}", userSupabaseId))
    ...
    @Override
    public Mono<TransactionDto> initiateOfflineDeposit(InitiateDepositRequest depositRequest, String userSupabaseId) {
...
//    }
    ...
    @Override
    public Mono<TransactionDto> withdraw(WithdrawRequestDto withdrawRequestDto, String userSupabaseId) {
        log.info("Initiating withdrawal for user: {}, amount: {}", userSupabaseId, withdrawRequestDto.getAmount());

        Mono<TransactionDto> withdrawMono = userProfileService.getUserProfileBySupabaseId(UUID.fromString(userSupabaseId))
                .switchIfEmpty(Mono.error(new ResourceNotFoundException("User profile not found")))
    ...
    @Override
    public Mono<TransactionDto> confirmWithdrawalByAdmin(String txnRef, String approverSupabaseId) {
...
//                                                wallet.setTotalAvailableBalance(wallet.getTotalAvailableBalance().subtract(amount));
    ...
    @Override
    public Mono<TransactionDto> rejectWithdrawalByAdmin(String txnRef, String reason, String approverSupabaseId) {
    ...
    @Override
    public Mono<TransactionDto> changeTransactionStatus(String txnRef, TransactionStatus status, String approverSupabaseId) {
                ...
                .flatMap(txn -> {
                    ...
                    if (status == TransactionStatus.COMPLETED) {
                        ...
                        if (txn.getTxnType() == TransactionType.WITHDRAWAL) {
                            return confirmWithdrawalByAdmin(txnRef, approverSupabaseId);
                        } else if (txn.getTxnType() == TransactionType.DEPOSIT) {
                            return confirmDepositOfflineByAdmin(txnRef, txn.getMetaData(), approverSupabaseId);
                        }

                    } else if (status == TransactionStatus.REJECTED && txn.getTxnType() == TransactionType.WITHDRAWAL) {
                        return rejectWithdrawalByAdmin(txnRef, "Rejected by admin", approverSupabaseId);
