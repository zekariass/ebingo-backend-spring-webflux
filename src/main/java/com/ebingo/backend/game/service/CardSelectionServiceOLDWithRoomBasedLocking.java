...
//@Slf4j
//public class CardSelectionServiceOLDWithRoomBasedLocking {
//
...
//        String userOwnedCardsKey = RedisKeys.userOwnedCardsKey(roomId, userId);
...
//                                                log.info("User {} successfully claimed card {} in room {}", userId, cardId, roomId)
//                                        ))
//                                        .then(publishSuccess(roomId, cardId, userId))
//                                );
//
//                        case "CARD_TAKEN" ->
...
//                })
//                .onErrorResume(err -> {
//                    log.error("Error claiming card {} for user {} in room {}: {}",
//                            cardId, userId, roomId, err.getMessage(), err);
//                    cleanupLocks(roomId, cardId, userId);
//                    return publishCardError(roomId, userId, cardId, "Internal server error", "UNKNOWN_ERROR");
//                });
//    }
//
//
...
//        String ownerKey = RedisKeys.cardOwnerKey(roomId, cardId);
//        String userOwnedKey = RedisKeys.userOwnedCardsKey(roomId, userId);
//        String cardLockKey = RedisKeys.cardLockKey(roomId, cardId);
//        String userLockKey = RedisKeys.userLockKey(roomId, userId);
//
...
//                    return switch (result) {
//                        case "OK" -> {
//                            log.info("User {} successfully released card {} in room {}", userId, cardId, roomId);
//                            yield handleSuccessfulRelease(roomId, gameId, userId, cardId);
//                        }
//                        case "NOT_OWNER" -> {
...
//    }
//
//    private Mono<Void> handleSuccessfulRelease(Long roomId, Long gameId, Long userId, String cardId) {
//        return Mono.when(
//                playerStateService.removePlayerCard(gameId, userId, cardId),
//                publishCardReleased(roomId, cardId, userId),
...
//
//
//    private Mono<Void> publishSuccess(Long roomId, String cardId, Long userId) {
...
//    }
//
//    private Mono<Void> publishCardReleased(Long roomId, String cardId, Long userId) {
//        return publisher.publishEvent(RedisKeys.roomChannel(roomId),
//                Map.of(
//                        "type", "cardReleased",
//                        "payload", Map.of(
...
//    }
//
//    private void cleanupLocks(Long roomId, String cardId, Long userId) {
//        String cardLockKey = RedisKeys.cardLockKey(roomId, cardId);
//        String userLockKey = RedisKeys.userLockKey(roomId, userId);
//
//        redis.delete(cardLockKey).subscribe(
//                null,
//                err -> log.warn("Failed to cleanup card lock for {}: {}", cardId, err.getMessage())
//        );
//        redis.delete(userLockKey).subscribe(
//                null,
//                err -> log.warn("Failed to cleanup user lock for {}: {}", userId, err.getMessage())
//        );
//    }
//
...
//        return publisher.publishUserEvent(roomId, userId,
//                Map.of(
//                        "type", "error",
//                        "payload", Map.of(
//                                "message", message,
