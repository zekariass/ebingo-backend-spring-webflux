...
public class BingoClaimServiceImpl implements BingoClaimService {
    ...
    @Override
    public Flux<BingoClaimDto> getPaginatedBingoClaim(String userSupabaseId, Integer page, Integer size, String sortBy) {
        ...
        return userProfileService.getUserProfileBySupabaseId(UUID.fromString(userSupabaseId))
                ...
                .doOnComplete(() -> log.info("Completed fetching past claims for user: {}", userSupabaseId))
                .doOnError(e -> log.error("Failed to fetch claims: {}", e.getMessage(), e));
    }

    @Override
    public Mono<BingoClaimDto> getBingoClaimById(String userSupabaseId, Long id) {
        return userProfileService.getUserProfileBySupabaseId(UUID.fromString(userSupabaseId))
                .switchIfEmpty(Mono.error(new ResourceNotFoundException("User profile not found")))
